...
const[toLogin,setToLogin]=useState(false)
 ...
 const handelSubmit =(e)=>{
  ...
  localStorage.setItem('domain',(signinstate.domain))
 ...
 const handelTologinClick =()=>{
  ...
  <h1>Registration Form Today NewsApp </h1>
  <div style={{ width: '60%', border: 'solid white', padding: '40px', borderRadius: '30px', textAlign: 'center' }}>
    <form onSubmit={handelSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
      <div style={{ display: 'flex', gap: '70px', justifyContent: 'center', alignItems: 'center' }}>
        <label style={{ fontSize: '20px' }} htmlFor="name">Full Name:</label>
        <input name='fname' onChange={handelChange} value={signinstate.fullname} required autoFocus className='form-control w-50 text-center' placeholder='Full Name' type="text" />
      </div>
      ...
      <div style={{ display: 'flex', gap: '90px', justifyContent: 'center', alignItems: 'center' }}>
        <label style={{ fontSize: '20px' }} htmlFor="domain">Domain:</label>
        <input  name='domainname' onChange={handelChange} value={signinstate.domain}  required className='form-control w-50 text-center' placeholder='Domain Developer' type="text" />
      </div>
